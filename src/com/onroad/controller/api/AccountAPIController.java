...
import com.onroad.common.bean.Constant;
import com.onroad.common.bean.DatumResponse;
import com.onroad.common.bean.LoginResponse;
import com.onroad.common.token.TokenManager;
import com.onroad.common.utils.DateUtils;
import com.onroad.common.utils.RandomUtils;
import com.onroad.common.utils.SMSUtils;
 ...
 * 用户账号相关的接口*
 *
 * 检查账号是否被注册: GET /api/account/checkUser 发送注册验证码: POST /api/account/sendCode 注册:
 * POST /api/account/register 登录： POST /api/account/login 查询用户资料: GET
...
public class AccountAPIController extends BaseAPIController {
	...
	public void sendCode() {
		...
		// 保存验证码数据
		RegisterCode registerCode = new RegisterCode().set(RegisterCode.MOBILE,
	...
	private void getProfile() {
		...
		if (StringUtils.isNotEmpty(userId)) {
			resultUser = user.findById(userId);
		} else {
			resultUser = getUser();
		}
	...
	private void updateProfile() {
		...
		if (flag) {
			...
			renderJson(response.setCode(update ? Code.SUCCESS : Code.FAIL)
					.setMessage(update ? "update success" : "update failed"));
		} else {
	...
	public void password() {
		...
		if (!notNull(Require.me().put(oldPwd, "old password can not be null")
				.put(newPwd, "new password can not be null"))) {
			return;
		}
		// 用户真实的密码
		User nowUser = getUser();
		if (StringUtils.encodePassword(oldPwd, "md5").equalsIgnoreCase(
				nowUser.getStr(PASSWORD))) {
			boolean flag = nowUser.set(User.PASSWORD,
					StringUtils.encodePassword(newPwd, "md5")).update();
	...
	public void avatar() {
		...
		String avatar = getPara("avatar");
		if (!notNull(Require.me().put(avatar, "avatar url can not be null"))) {
			return;
		}
