...
public class VersionProperty {
	...
	private static final Logger Log = LoggerFactory
			.getLogger(VersionProperty.class);
	 ...
	 */
	/*
	 * public VersionProperty(InputStream in) throws IOException { if (in !=
	 * null) { Reader reader = new BufferedReader(new InputStreamReader(in,
	 * "UTF-8")); buildDoc(reader); } }
	 */
		...
		if (!file.exists()) {
			...
			// being there, so throw an error.
				...
				throw new FileNotFoundException(
						"XML properties file does not exist: " + file.getName());
			}
		}
	...
	private void buildNowVersion() {
		nowVersion.put(ClientType.ANDROID.getType(),
				loadNowVersion(ClientType.ANDROID));
		nowVersion.put(ClientType.IPHONE.getType(),
				loadNowVersion(ClientType.IPHONE));
	}
	...
	private synchronized void saveProperties() {
		...
		if (!error) {
			...
			if (!file.delete()) {
				Log.error("Error deleting property file: "
	...
	public static void main(String[] args) {
		try {
			...
			List<Version> androidVersions = property
					.getVersions(ClientType.ANDROID);
